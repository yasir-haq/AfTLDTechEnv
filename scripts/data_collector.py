        ...
        try:
            ...
            for resolver in self.recursive_servers:
                ...
                if result.status == getdns.RESPSTATUS_GOOD:
                    ...
                    self.result[tld][resolver[0]['address_data']] = list(ns_list)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-k', '--kind', help='Kind of measurement', required=True, choices=["afri", "top_cctld"])
    ...
    if os.path.isfile(nro_alloactions_file) and os.path.isfile(anycast_dataset_caida_file) and os.path.isfile(anycast_dataset_greedy_file):
        ...
        df = pd.DataFrame(columns=['cc','dnssec', 'dnssec_algo', 'deploy360', 'ns', 'authoritative', 'ip', 'af', 'asn4', 'asn6','region','anycast_provider', 'anycast_provider_ratio'])
        ...
        df.to_csv(results_folder + '/' + kind + '_cctld_ns.csv')
